                         ...
                         enum nx_packet_in_format packet_in_format)
{
     ...
     */
    //dpid
    char *dpid = (char*)malloc(sizeof(char)*13);
    char *binary = (char*)malloc(sizeof(char)*49);
    char *binary_tmp = (char*)malloc(sizeof(char)*49);
    char* switch_no_char = (char *)malloc(sizeof(long int) + 1);
    //portid
    char *portid = (char*)malloc(sizeof(char)*5);
    char *binary_port = (char*)malloc(sizeof(char)*17);
    char *binary_port_tmp = (char*)malloc(sizeof(char)*17);
    char* port_no_char = (char *)malloc(sizeof(int) + 1);
    //dump bandwidth
    char *dump_bandwidth = (char*)malloc(sizeof(char)*40);
    char *bandwidth_return = (char*)malloc(sizeof(char)*1000);
    char *bandwidth = (char*)malloc(sizeof(char)*10);
    bandwidth[0]='\0';
    //dump delay jitter loss
    char *dump_other = (char*)malloc(sizeof(char)*40);
    char *other_return = (char*)malloc(sizeof(char)*1000);
    char *delay = (char*)malloc(sizeof(char)*10);
    char *jitter = (char*)malloc(sizeof(char)*10);
    char *loss = (char*)malloc(sizeof(char)*10);
    delay[0]='\0';
    jitter[0]='\0';
    int i,j,k;
    long int  switch_no = 0;
    int port_no = 0;
    long int atom = 1;
    int switch_no_char_size;
    int port_no_char_size;
    int bandwidth_return_size;
    int other_return_size;
    unsigned short int ibandwidth;
    unsigned short int idelay;
    unsigned short int ijitter;
    unsigned short int iloss;
    if(packet->size==148){
//    	VLOG_INFO("lldp in packet_in: %s",ofpbuf_to_string(packet,packet->size));
    	//dpid
//    	for(i=0;i<5;i++){
//    		memcpy(dpid+i*2,ofpbuf_to_string(packet,packet->size)+258+i*3,2);
//    	}
//    	memcpy(dpid+10,ofpbuf_to_string(packet,packet->size)+283,2);
//    	dpid[12]='\0';
//    	VLOG_INFO("dpid: %s",dpid);
    	//binary_dpid
//    	for(i=0;i<48;i++) binary[i]='0';
//    	for(i=0,j=0;i<12;i++,j=j+4){
//    		if(dpid[i]=='1') binary[j+3]='1';
//    		else if(dpid[i]=='2') binary[j+2]='1';
//    		else if(dpid[i]=='3') binary[j+3]='1',binary[j+2]='1';
//    		else if(dpid[i]=='4') binary[j+1] = '1';
//    		else if(dpid[i]=='5') binary[j+3] = '1',binary[j+1]='1';
//    		else if(dpid[i]=='6') binary[j+2] = '1',binary[j+1]='1';
//    		else if(dpid[i]=='7') binary[j+3] = '1',binary[j+2]='1',binary[j+1]='1';
//    		else if(dpid[i]=='8') binary[j] = '1';
//    		else if(dpid[i]=='9') binary[j+3]='1',binary[j]='1';
//    		else if(dpid[i]=='a') binary[j+2]='1',binary[j]='1';
//    		else if(dpid[i]=='b') binary[j+3]='1',binary[j+2]='1',binary[j]='1';
//    		else if(dpid[i]=='c') binary[j+1]='1',binary[j]='1';
//    		else if(dpid[i]=='d') binary[j+3]='1',binary[j+1]='1',binary[j]='1';
//    		else if(dpid[i]=='e') binary[j+2]='1',binary[j+1]='1',binary[j]='1';
//    		else if(dpid[i]=='f') binary[j+3]='1',binary[j+2]='1',binary[j+1]='1',binary[j]='1';
//    	}
//    	binary[48]='\0';
    	//binary_dpid_tmp
//    	for(i=0,j=47;i<48;i++,j--){
//    		binary_tmp[i] = binary[j];
//    	}
//    	binary_tmp[48]='\0';
    	//switch_no
//    	if(binary_tmp[0]=='1') switch_no +=1;
//    	for(i=1;i<48;i++){
//    		if(binary_tmp[i]=='1'){
//    			atom = 1;
//    			for(j=1;j<=i;j++) atom *=2;
//    			switch_no += atom;
//    		}
//    	}
    	//switch_no_char,switch_no_char_size
//    	memset(switch_no_char, 0, sizeof(long int) + 1);
//    	sprintf(switch_no_char, "%ld", switch_no);
//    	for(i=0;switch_no_char[i]!='\0';i++){
//    	}
//    	switch_no_char_size = i;
//    	VLOG_INFO("switch_no_char: %s, switch_no_char_size: %d",switch_no_char,switch_no_char_size);
    	//portid
//    	memcpy(portid,ofpbuf_to_string(packet,packet->size)+295,2);
//    	memcpy(portid+2,ofpbuf_to_string(packet,packet->size)+298,2);
//    	portid[4]='\0';
//    	VLOG_INFO("portid: %s",portid);
    	//binary_port
//    	for(i=0;i<16;i++) binary_port[i]='0';
//    	for(i=0,j=0;i<4;i++,j=j+4){
//    		if(portid[i]=='1') binary_port[j+3]='1';
//    		else if(portid[i]=='2') binary_port[j+2]='1';
//    		else if(portid[i]=='3') binary_port[j+3]='1',binary_port[j+2]='1';
//    		else if(portid[i]=='4') binary_port[j+1] = '1';
//    		else if(portid[i]=='5') binary_port[j+3] = '1',binary_port[j+1]='1';
//    		else if(portid[i]=='6') binary_port[j+2] = '1',binary_port[j+1]='1';
//    		else if(portid[i]=='7') binary_port[j+3] = '1',binary_port[j+2]='1',binary_port[j+1]='1';
//    		else if(portid[i]=='8') binary_port[j] = '1';
//    		else if(portid[i]=='9') binary_port[j+3]='1',binary_port[j]='1';
//    		else if(portid[i]=='a') binary_port[j+2]='1',binary_port[j]='1';
//    		else if(portid[i]=='b') binary_port[j+3]='1',binary_port[j+2]='1',binary_port[j]='1';
//    		else if(portid[i]=='c') binary_port[j+1]='1',binary_port[j]='1';
//    		else if(portid[i]=='d') binary_port[j+3]='1',binary_port[j+1]='1',binary_port[j]='1';
//    		else if(portid[i]=='e') binary_port[j+2]='1',binary_port[j+1]='1',binary_port[j]='1';
//    		else if(portid[i]=='f') binary_port[j+3]='1',binary_port[j+2]='1',binary_port[j+1]='1',binary_port[j]='1';
//    	}
//    	binary_port[16]='\0';
//    	VLOG_INFO("binary_port: %s",binary_port);
    	//binary_portid_tmp
//    	for(i=0,j=15;i<16;i++,j--){
//    		binary_port_tmp[i] = binary_port[j];
//    	}
//    	binary_port_tmp[16]='\0';
//    	VLOG_INFO("binary_port_tmp: %s",binary_port_tmp);
    	//port_no
//    	if(binary_port_tmp[0]=='1') port_no +=1;
//    	for(i=1;i<16;i++){
//    		if(binary_port_tmp[i]=='1'){
//    			atom = 1;
//    			for(j=1;j<=i;j++) atom *=2;
//    			port_no += atom;
//    		}
//    	}
//    	//port_no_char,port_no_char_size
//    	memset(port_no_char, 0, sizeof(int) + 1);
//    	sprintf(port_no_char, "%d", port_no);
//    	for(i=0;port_no_char[i]!='\0';i++){
//    	}
//    	port_no_char_size = i;
//    	VLOG_INFO("port_no_char: %s, port_no_char_size: %d",port_no_char,port_no_char_size);
    	//dump bandwidth
//    	char * head = "tc class show dev s";
//    	char * middle = "-eth";
//    	memcpy(dump_bandwidth,head,19);
//    	memcpy(dump_bandwidth+19,switch_no_char,switch_no_char_size);
//    	memcpy(dump_bandwidth+19+switch_no_char_size,middle,4);
//    	memcpy(dump_bandwidth+23+switch_no_char_size,port_no_char,port_no_char_size);
//    	dump_bandwidth[23+switch_no_char_size+port_no_char_size] = '\0';
//    	for(i=0;uciget(dump_bandwidth)[i]!='\0';i++){
//    	}
//    	bandwidth_return_size = i;//如果在没有设置带宽的情况下通常为0
//    	memcpy(bandwidth_return,uciget(dump_bandwidth),i);
//    	bandwidth_return[i]='\0';
//    	VLOG_INFO("bandwidth_return_size: %d",bandwidth_return_size);
//    	VLOG_INFO("dump_bandwidth: %s",dump_bandwidth);
//    	VLOG_INFO("bandwidth_return: %s",bandwidth_return);
    	//下面这部分代码：在设置带宽与否之间进行选择
//    	int bandwidth_bool = 0;
//    	for(i=0;i<bandwidth_return_size;i++){
//    		if(bandwidth_return[i]=='M'&&bandwidth_return[i+1]=='b'&&
//    				bandwidth_return[i+2]=='i'&&bandwidth_return[i+3]=='t'){
//    			bandwidth_bool = 1;
//    			for(j=i-1;bandwidth_return[j]!=' ';j--){
//    			}
//    			for(k=j+1;k<i;k++){
//    				bandwidth[k-j-1]=bandwidth_return[k];
//    			}
//    			bandwidth[k-j-1]='\0';
//    			break;
//    		}
//    	}
//    	VLOG_INFO("bandwidth:%s",bandwidth);
//    	if(bandwidth_bool>0){
//			ibandwidth = atoi(bandwidth);
//			memset(packet->data+80,ibandwidth >> 8,1);
//			memset(packet->data+81,ibandwidth & 0x00ff,1);
//			VLOG_INFO("bandwidth:%d",ibandwidth);
//    	}
    	// dump delay jitter loss
//    	char * head_other = "tc qdisc show dev s";
//    	memcpy(dump_other,head_other,19);
//    	memcpy(dump_other+19,switch_no_char,switch_no_char_size);
//    	memcpy(dump_other+19+switch_no_char_size,middle,4);
//    	memcpy(dump_other+23+switch_no_char_size,port_no_char,port_no_char_size);
//    	dump_other[23+switch_no_char_size+port_no_char_size] = '\0';
//    	for(i=0;uciget(dump_other)[i]!='\0';i++){
//    	}
//    	other_return_size = i;
//    	memcpy(other_return,uciget(dump_other),i);
//    	other_return[i]='\0';
//    	VLOG_INFO("dump_other: %s",dump_other);
//    	VLOG_INFO("other: %s",other_return);
    	//下面这段代码是寻找延迟
//    	int delay_bool=0;
//    	int u_bool = 1;//当u_bool=1是表示单位us,当为u=0时，表示ms
//    	for(i=0;i<other_return_size;i++){
//    		if(other_return[i]=='d'&&other_return[i+1]=='e'&&
//    				other_return[i+2]=='l'&&other_return[i+3]=='a'&&other_return[i+4]=='y'){
//    				delay_bool=1;
//        			for(j=i;;j++){
//        				if(other_return[j]=='u') break;
//        				else if(other_return[j]=='m') {
//        					u_bool = 0;
//        					break;
//        				}
//        			}
//        			for(k=i+6;k<j;k++){
//        				delay[k-i-6]=other_return[k];
//        			}
//        			delay[k-i-6]='\0';
//        			break;
//    		}
//    	}
//    	VLOG_INFO("delay:%s",delay);
    	//设置延迟
//    	if(delay_bool>0){
//    		if(u_bool<1){
//    			idelay = atoi(delay)*1000;
//    		}
//    		else{
//    			idelay = atoi(delay);
//    		}
//			memset(packet->data+88,idelay >> 8,1);
//			memset(packet->data+89,idelay & 0x00ff,1);
//    	}
    	//下面这段代码寻找抖动
//    	int jitter_bool = 0;
//    	int m_bool = 0;//表示是us
//    	for(i=0;i<other_return_size;i++){
//    		if(other_return[i]==' '&&other_return[i+1]==' '){
//    			jitter_bool = 1;
//    			for(j=i;;j++){
//    				if(other_return[j]=='u') break;
//    				else if(other_return[j]=='m') {
...
//    			}
//    			for(k=i+2;k<j;k++){
//    				jitter[k-i-2]=other_return[k];
//    			}
//    			jitter[k-i-2]='\0';
//    			break;
//    		}
//    	}
//    	VLOG_INFO("jitter: %s",jitter);
    	//设置jitter
//    	if(jitter_bool>0){
//    		if(m_bool<1){
//    			ijitter = atoi(jitter);
//    		}
//    		else{
//    			ijitter = atoi(jitter)*1000;
//    		}
//			memset(packet->data+96,ijitter >> 8,1);
//			memset(packet->data+97,ijitter & 0x00ff,1);
//    	}
    	//下面的代码用来寻找丢包率
//    	int loss_bool = 0;
//    	for(i=0;i<other_return_size;i++){
//    		if(other_return[i]=='l'&&other_return[i+1]=='o'&&
//    				other_return[i+2]=='s'&&other_return[i+3]=='s'){
//    			loss_bool = 1;
//    			for(j=i;;j++){
//    				if(other_return[j]=='%') break;
//    			}
//    			for(k=i+5;k<j;k++){
//    				loss[k-i-5]=other_return[k];
//    			}
//    			loss[k-i-5]='\0';
//    			break;
//    		}
...
//    	if(loss_bool>0){
//    		iloss = atoi(loss);
//			memset(packet->data+104,iloss,1);
//    	}
    ...
    free(portid);
    free(binary_port);
    free(binary_port_tmp);
    free(port_no_char);
    //dump bandwidth
    free(dump_bandwidth);
    free(bandwidth_return);
    free(bandwidth);
    //dump delay jitter loss
    free(dump_other);
    free(other_return);
