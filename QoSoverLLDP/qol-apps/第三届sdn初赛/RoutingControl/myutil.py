...
DynamicOperateFlows() 动态流表的操作包括删除和添加，这个方法很强大
"""
...
#这是一个批量动态操作流表的方法       
def DynamicOperateFlows(network,pathsrctodst,action):
    '''
    这个方法实现了从拓扑动态获取信息，生成静态流表并推送的过程
    '''
    def addFlow(hp,sp):
        in_outputdict,srcname,dstname = in_portOutputIp(switches,hosts,sp,hp)
        path = srctodstlink(switches+hosts,srcname,dstname,sp)
        srcip = network.get(srcname).IP()
        dstip = network.get(dstname).IP()
        pathlist = path.split(':')
        pathlist = pathlist[1:len(pathlist)-1]
        pathstr = ''
        for path in pathlist:
            pathstr += path
        for d in in_outputdict:
            inoutput = d+'#'+d[0:len(d)-1]+in_outputdict[d][0]
            if(re.search(inoutput,pathstr) is not None):
                in_outputdict[d].append([srcip,dstip])
            else:
                in_outputdict[d].append([dstip,srcip])
        StaticFlows(switches,action=action,in_outputdict=in_outputdict,srcip=srcip,
                dstip=dstip,hostspath=hp)#添加静态流表
    
    switches = network.switches
    hosts = network.hosts               
    for path in pathsrctodst:
        addFlow(path,pathsrctodst[path])

#这是一个动态操作流表的方法       
def DynamicOperateFlow(network,hostspath,switchespath,action):
    '''
    这个方法实现了从拓扑动态获取信息，生成静态流表并推送的过程
    '''
    def addFlow(hp,sp):
    ...
    switches = network.switches
    hosts = network.hosts               
    addFlow(hostspath,switchespath)
