 ...
 * id as well as the outgoing port number.
 * 这个类发出的LLDP消息包含发送的交换机的dpid以及传出的端口号。
 * 
 *  [主语](Received LLrescDP messages that (match a known switch)) [谓语]cause [宾语] a new 
 *  LinkTuple to be created [状语](according to the invariant rules listed below. )
 * 根据下面列出的不变的规则，收到匹配一个已知的交换机引起一个新的链接元组被创建。
 * 
 * This new LinkTuple is also passed to routing if it exists to trigger updates. 
 *  如果存在触发更新，这个新的链接元组也将被传递去路由。
 *  
 * This class also handles [宾语](removing links that are associated to switch ports that 
 * go down, and switches that are disconnected.)
 * 这个类也处理移除关联到交换机关闭的端口的链接和断开连接的交换机。
 * 
 * Invariants: 
 * 不变量：
 * -portLinks and switchLinks will not contain empty Sets outside of critical sections 
 * -端口的链接和交换机的链接将不包含关键部分外的空集
 * 
 * -portLinks contains LinkTuples [定语从句](where one of the src or dst SwitchPortTuple matches the map key)
 * -端口链接包含的链接元组，它的或目的交换机的端口元组匹配映射的关键字
 * 
 * -switchLinks contains LinkTuples where one of the src or dst SwitchPortTuple's id matches the switch id 
 * -交换机链接包含的链接元组，它的任意一个源或目的交换机元组的id匹配交换机的id
 * 
 * -Each LinkTuple will be indexed into switchLinks for both src.id and dst.id, and portLinks for each src and dst 
 * -每个链接元组将被索引到交换机链接为源id和目的id,和端口链接为每个源和目的
 * 
 * -The updates queue is only added to from within a held write lock
 * 
 * 注意：为了注释的方便，把LinkDiscoveryManager简写成LDM
 * 
...
IFloodlightModule, IInfoProvider {
	protected static final Logger log = LoggerFactory.getLogger(LinkDiscoveryManager.class);//为LDM注册一个名为log的日志仪
	protected static final INotificationManager notifier =
			NotificationManagerFactory.getNotificationManager(LinkDiscoveryManager.class);//为LDM注册一个名为notifier的通知管理器

	public static final String MODULE_NAME = "linkdiscovery";//定义一个字符串来存放模块的名称标识

	// Names of table/fields for links in the storage API
	//storage API 里面的表的名字/字段的名字
	private static final String TOPOLOGY_TABLE_NAME = "controller_topologyconfig";//定义一个字符串来存放表示拓扑的表的名称
	private static final String TOPOLOGY_ID = "id";//定义一个字符串来存放拓扑的id
	private static final String TOPOLOGY_AUTOPORTFAST = "autoportfast";//定义一个字符串来存放表示拓扑自动快速端口
	...
	private static final String SWITCH_CONFIG_TABLE_NAME = "controller_switchconfig";//交换机的存放配置的表的名称的标识

	// Event updaters for debug events
	//调试事件的事件更新器
	protected IEventCategory<DirectLinkEvent> eventCategory;//事件类别接口

	protected IFloodlightProviderService floodlightProviderService;//floodlight提供服务的接口
	protected IOFSwitchService switchService;//交换机的服务接口
	protected IStorageSourceService storageSourceService;//存储源服务的接口
	protected IThreadPoolService threadPoolService;//线程池服务的接口
	protected IRestApiService restApiService;//restApi服务的接口
	protected IDebugCounterService debugCounterService;//调试计数器服务的接口
	protected IDebugEventService debugEventService;//调试事件服务的接口
	protected IShutdownService shutdownService;//关闭服务的接口
	// Role
	//扮演的角色
	protected HARole role;//给定的控制节点在SDN网络管理扮演的角色

	// LLDP and BDDP fields
	//LLDP和BDDP字段
	private static final byte[] LLDP_STANDARD_DST_MAC_STRING = //LLDP标准的目的mac地址串
			HexString.fromHexString("01:80:c2:00:00:0e");
	private static final long LINK_LOCAL_MASK = 0xfffffffffff0L;//本地链路的掩码
	private static final long LINK_LOCAL_VALUE = 0x0180c2000000L;//本地链路的值
	protected static int EVENT_HISTORY_SIZE = 1024; // in seconds 历史事件的长度

	// BigSwitch OUI is 5C:16:C7, so 5D:16:C7 is the multicast version
	//BigSwitch的开放统一接口是5C:16:c7,所以5D:16:C7是组播的版本
	// private static final String LLDP_BSN_DST_MAC_STRING =
	// "5d:16:c7:00:00:01";
	private static final String LLDP_BSN_DST_MAC_STRING = "ff:ff:ff:ff:ff:ff";//暂时未知

	// Direction TLVs are used to indicate if the LLDPs were sent
	// 类型长度值字段被用来表明是否LLDP被发送了
	// periodically or in response to a recieved LLDP
	// 周期性地或响应一个收到的LLDP
	private static final byte TLV_DIRECTION_TYPE = 0x73;//TLV方向的类型
	private static final short TLV_DIRECTION_LENGTH = 1; // 1 byte TLV方向的长度
	...
	private static final LLDPTLV forwardTLV = new LLDPTLV().setType(TLV_DIRECTION_TYPE)
			...
			.setValue(TLV_DIRECTION_VALUE_FORWARD);//转发的TLV

	private static final LLDPTLV reverseTLV = new LLDPTLV().setType(TLV_DIRECTION_TYPE)
			.setLength(TLV_DIRECTION_LENGTH)
			.setValue(TLV_DIRECTION_VALUE_REVERSE);//相反的TLV

	// Link discovery task details.链路发现任务的细节
	protected SingletonTask discoveryTask;//发现任务是一个类
	protected final int DISCOVERY_TASK_INTERVAL = 1;//发现任务的间隔
	protected final int LINK_TIMEOUT = 35; // timeout as part of LLDP process.链路超时
	protected final int LLDP_TO_ALL_INTERVAL = 15; // 15 seconds.LLDP的到全部的间隔
	protected long lldpClock = 0;//lldp的时钟
	// This value is intentionally kept higher than LLDP_TO_ALL_INTERVAL.
	//下面的这个值是为了保持比LLDP_TO_ALL_INTERVAl更加高的事件间隔
	// If we want to identify link failures faster, we could decrease this value to a small number, say 1 or 2 sec.
	//如果我们想要指定的链路更快的出现错误，我们可以减小这个值，比如说1秒或2秒
	protected final int LLDP_TO_KNOWN_INTERVAL = 20; // LLDP frequency for known
	// links
	//链路
	protected LLDPTLV controllerTLV;//控制器的TLV
	protected ReentrantReadWriteLock lock;//一把钥匙
	int lldpTimeCount = 0;//lldp的超时计数

	/*
	 * Latency tracking
	 * 延迟跟踪
	 */
	protected static int LATENCY_HISTORY_SIZE = 10;//延迟的历史大小
	protected static double LATENCY_UPDATE_THRESHOLD = 0.50;//延迟更新的阈值

	/**
	 * Flag to indicate if automatic port fast is enabled or not. 
	 * 指示自动快速端口开启与否的标志
	 * Default is set to false -- Initialized in the init method as well.
	 * 默认情况下是设置为false -- 在init的方法中初始化也是这样设置的
	 */
	protected boolean AUTOPORTFAST_DEFAULT = false;//自动的快速端口的默认开启状态
	protected boolean autoPortFastFeature = AUTOPORTFAST_DEFAULT;//自动的快速端口的特征，相当与以前经常写的temp

	/**
	 * Map from link to the most recent time 状语从句(when)it was verified functioning
	 * 从链接到最近一个被验证功能的映射
	 */
	 ...
	 * 从交换机到所有链路集合(以该交换机作为端点)的映射
	 */
	protected Map<DatapathId, Set<Link>> switchLinks;

	/**
	 * Map from a id:port to the set of links containing it as an endpoint
	 * 从一个 id和port的元组到一个链路集合（id和port所对应的一个节点是一个端点）的映射
	 */
	protected Map<NodePortTuple, Set<Link>> portLinks;

	protected volatile boolean shuttingDown = false;//关闭的状态

	/*
	 * topology aware components are called in the order they were added to the array
	 * 拓扑感知组件在他们被添加到数组的顺序里面被调用
	 */
	protected ArrayList<ILinkDiscoveryListener> linkDiscoveryAware;//链路发现的感知
	protected BlockingQueue<LDUpdate> updates;//更新的队列
	protected Thread updatesThread;//更新的线程

	/**
	 * List of ports through which LLDP/BDDPs are not sent.
	 * 列出哪一些端口没有发送LLDP/BDDPs的列表
	 */
	protected Set<NodePortTuple> suppressLinkDiscovery;//钳制的链路发现

	/**
	 * A list of ports that are quarantined for discovering links through them.
	 * 一个被隔离去通过它们发现链路的端口的列表
	 * Data traffic from these ports are not allowed until the ports are released from quarantine.
	 * 直到端口从隔离的列表里面释放出来，这些端口才被允许有数据通过
	 */
	protected LinkedBlockingQueue<NodePortTuple> quarantineQueue;//被隔离的队列
	protected LinkedBlockingQueue<NodePortTuple> maintenanceQueue;//被维持的队列
	protected LinkedBlockingQueue<NodePortTuple> toRemoveFromQuarantineQueue;//从隔离队列移除的元组的队列
	protected LinkedBlockingQueue<NodePortTuple> toRemoveFromMaintenanceQueue;//从维持队列移除的元组的队列

	/**
	 * Quarantine task
	 * 隔离的任务
	 */
	protected SingletonTask bddpTask;//bddp任务
	protected final int BDDP_TASK_INTERVAL = 100; // 100 ms.bddp任务的时间间隔
	protected final int BDDP_TASK_SIZE = 10; // # of ports per iteration bddp任务的大小

	private class MACRange {
		MacAddress baseMAC;
		int ignoreBits;
	}
	protected Set<MACRange> ignoreMACSet;//忽略的MAC地址的集合
	 ...
	 * 调试信息的计数器
	 */
	private IDebugCounter ctrQuarantineDrops;//隔离队列的丢弃的计数器
	private IDebugCounter ctrIgnoreSrcMacDrops;
	private IDebugCounter ctrIncoming;//packet_in的计数器
	private IDebugCounter ctrLinkLocalDrops;
	private IDebugCounter ctrLldpEol;//未知含义
	...
	public OFPacketOut generateLLDPMessage(IOFSwitch iofSwitch, OFPort port,
			boolean isStandard, boolean isReverse) {//isReverse 表示是否发过去一个包要发回来的标志
		//openflow端口的描述（description）
		OFPortDesc ofpPort = iofSwitch.getPort(port);//通过传进来的端口号从交换机上面弄出一个端口对象
		...
		//参考文献：
		//http://wenku.baidu.com/link?url=TNDLHdTEmK6oghaOe1UjJvP_FJgH25bwdgjgKhOiRzLtUFOEAGyDaepKb4bKZ-utTbWp1K2Be3ZJhCkreanH71oVANDbnNcX6_Uyv4pjRsC
		//这里的chassisID 里面的第一个4表示mac地址，后面的0表示保留的，下面这个整句代码的含义就是：
		//定义一个类型为字节的机箱id的数组，第一个字节表示类型为mac地址，后面6个字节用来放置mac地址（48位）
		byte[] chassisId = new byte[] { 4, 0, 0, 0, 0, 0, 0 }; // filled in later 机箱ID
		//定义一个类型为字节的端口id的数组，第一个字节表示类型为端口组件，后面2个字节用来表示端口号（16位）
		byte[] portId = new byte[] { 2, 0, 0 }; // filled in later 端口ID
		//下面这两个字节都是生存时间的值，默认为120
		byte[] ttlValue = new byte[] { 0, 0x78 };
		/**
		 * @author wjl modify start
		 */
		...
		/**
		 * @author wjl modify end
		 */
		// OpenFlow OUI - 00-26-E1-00 (Organizationally Unique Identifier)
		...
		//TODO:这个块的作用就是产生一个伪装的mac,实际上我认为这个块的代码一点都不优雅
		// set the chassis id's value to last 6 bytes of dpid
		//把机箱的mac地址的值赋值给dpidArray的数组的
		...
		//考虑移除下面这块代码
		// It's evil to overwrite port object.
		//重写端口对象是不好的
		//The the old code always overwrote mac address, we now only overwrite zero macs and log a warning, mostly for paranoia.
		//这个老的代码总是重写mac地址，现在我们只重写全为0的mac地址并记录警告，大多数情况下是偏执的。
		...
		if (Arrays.equals(srcMac, zeroMac)) {
			log.warn("Port {}/{} has zero hareware address"
					...
					dpid.toString(), ofpPort.getPortNo().getPortNumber());//在终端显示警告
			System.arraycopy(dpidArray, 2, srcMac, 0, 6);//生成一个伪装的mac
		}
		 ...
		 * 介绍一个新的TLV，为了med-granularity 链路的延迟检测。
		 * If same controller, can assume system clock is the same, but cannot guarantee processing time or account for network congestion.
		 * 如果一个相同的控制器，可以假设系统的事件事件是相同的，但是不能保证进程的时间或交付的网络拥塞
...
//		log.info("\n收到一个LLDP数据包:"+qos.toString());
//		Collection<Qos> qosvalues = (Collection<Qos>) qosdatabase.values();
//		String strqosvalues = new String();
//		for( Qos q : qosvalues){
//			strqosvalues += q.toString();
//		}
//		log.info("\n当前qos的数据库里面存有如下Qos信息:\n"+strqosvalues);
