...
{
//	char *lldppacket = dp_packet_to_string(frame, 108);
//    char *first = "echo ";
//    char *second = " > /home/wjl/hello.txt";
//    char *res = NULL;
//    res = strcontact(first,lldppacket);
//    res = strcontact(res,second);
//    system(res);
//    free(res);
//    res = NULL;

    struct lldpd_chassis *chassis;//机箱
    struct lldpd_port *port;//端口
    const char lldpaddr[] = LLDP_MULTICAST_ADDR;//多播地址
    /**
     * wjl modify
     */
    const char wjl[] = LLDP_TLV_ABCDEFG;
     ...
     * wjl modify
     */
    bool bandwidth_received = false;
    bool delay_received = false;
    bool jitter_received = false;
    bool loss_received = false;
    ...
    if (length < 2 * ETH_ADDR_LEN + sizeof(u_int16_t)) {
        ...
        system("echo length2*ETH_ADDR_LEN+sizeof(u_int16_t) > /home/wjl/length.txt");
        goto malformed;
    }
    if (PEEK_CMP(lldpaddr, ETH_ADDR_LEN) != 0) {
        VLOG_INFO("frame not targeted at LLDP multicast address "
                  "received on %s", hardware->h_ifname);
        system("echo PEEK_CMP(lldpaddr, ETH_ADDR_LEN)0 > /home/wjl/peekcmp.txt");
        goto malformed;
    }
    ...
    if (PEEK_UINT16 != ETHERTYPE_LLDP) {
        ...
        system("echo PEEK_UINT16ETHERTYPE_LLDP > /home/wjl/peekuint16.txt");
        goto malformed;
    }
    ...
    while (length && !gotend) {
        if (length < 2) {
            VLOG_WARN("tlv header too short received on %s",
                      hardware->h_ifname);
            system("echo length2 > /home/wjl/length2.txt");
            goto malformed;
        }
        ...
        if (length < tlv_size) {
            VLOG_WARN("frame too short for tlv received on %s",
                      hardware->h_ifname);
            system("echo lengthtlvsize > /home/wjl/length3.txt");
            goto malformed;
        }
        ...
        case LLDP_TLV_END:
            if (tlv_size != 0) {
                VLOG_WARN("lldp end received with size not null on %s",
                          hardware->h_ifname);
                system("echo tlvsize0 > /home/wjl/tlvsize.txt");
                goto malformed;
            }
            if (length) {
        ...
        case LLDP_TLV_PORT_ID:
            ...
            if (tlv_subtype == 0 || tlv_subtype > 7) {
                VLOG_WARN("unknown subtype for tlv id received on %s",
                          hardware->h_ifname);
                system("echo tlvsubtype > /home/wjl/tlvsubtype.txt");
                goto malformed;
            }
        ...
        case LLDP_TLV_MGMT_ADDR:
            ...
            if (mgmt == NULL) {
                ...
                system("echo mgmtnull > /home/wjl/mgmt.txt");
                goto malformed;
            }
        ...
        case LLDP_TLV_ORG:
            ...
            if (memcmp(dot1, orgid, sizeof orgid) == 0) {
                ...
                /* LLDP-MED */
                hardware->h_rx_unrecognized_cnt++;
            } else if (memcmp(wjl, orgid, sizeof orgid) == 0){
            	/**
            	 * wjl modify
            	 */
            	switch(tlv_subtype){
                ...
                case LLDP_TLV_AA_ISID_VLAN_ASGNS_SUBTYPE:
                    ...
                    if (num_mappings % 5 != 0) {
                        ...
                        system("echo nummapping > /home/wjl/num.txt");
                        goto malformed;
                    }
        ...
        default:
            VLOG_WARN("unknown tlv (%d) received on %s",
                      tlv_type,
                      hardware->h_ifname);
            system("echo default > /home/wjl/default.txt");
            goto malformed;
        }
        if (pos > tlv + tlv_size) {
            VLOG_WARN("BUG: already past TLV!");
            system("echo postlvtlvsize > /home/wjl/pos.txt");
            goto malformed;
        }
    ...
    if (!chassis->c_id || !port->p_id || !ttl_received ||!gotend) {
    	if(!chassis->c_id){
    		system("echo > /home/wjl/chassiscid.txt");
    	}
    	else if(!port->p_id){
    		system("echo > /home/wjl/portid.txt");
    	}
    	else if(!ttl_received){
    		system("echo > /home/wjl/ttl.txt");
    	}
    	else if(!gotend){
    		system("echo > /home/wjl/gotend.txt");
    	}
        VLOG_WARN("some mandatory tlv are missing for frame received "
                  "on %s", hardware->h_ifname);
        system("echo check > /home/wjl/check123.txt");
        goto malformed;
    }
    *newchassis = chassis;
    *newport = port;
