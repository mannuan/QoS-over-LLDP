 ...
 *wjl modify
 */
#define LLDP_CHASSIS_BANDWIDTH	 0x6666
#define LLDP_CHASSIS_DELAY		0x6666
#define LLDP_CHASSIS_JITTER	    0x6666
#define LLDP_CHASSIS_LOSS		0x66
...
{
	struct lldp *lldp;//lldp结构体
    struct lldpd_chassis *lchassis;//lldp数据的机箱
    struct lldpd_hardware *hw;//lldp数据的硬件
    struct aa_mapping_internal *m;//自动附加的内部映射

    if (!cfg || !smap_get_bool(cfg, "enable", false)) {
        return NULL;
    }

    lldp = xzalloc(sizeof *lldp);//为lldp分配空间
     ...
     * wjl modify
     */
    lchassis->bandwidth = LLDP_CHASSIS_BANDWIDTH;
    lchassis->delay = LLDP_CHASSIS_DELAY;
    lchassis->jitter = LLDP_CHASSIS_JITTER;
    lchassis->loss = LLDP_CHASSIS_LOSS;
