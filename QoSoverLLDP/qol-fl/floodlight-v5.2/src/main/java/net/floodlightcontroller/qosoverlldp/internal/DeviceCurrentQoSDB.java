...
public class DeviceCurrentQoSDB extends HashMap<String,DeviceCurrentQoS> {

    protected boolean updateDeviceQoS(String devicename,DeviceCurrentQoS qos){
        put(devicename, qos);
        return true;
    }

    public boolean isAllDeviceQoSareAllUpdated(long currenttime){
        for(String device:keySet()){
            if(get(device).getTime()<currenttime){
                return false;
            }
        }
        return true;
