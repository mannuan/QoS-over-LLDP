...
public class DeviceHistoryQoSListDB extends HashMap<String,HashMap<String,ArrayList<Object>>>{
    ...
    public static String VISUALTIME = "visualtime";
    public static String VISUALBANDWIDTH = "visualbandwidth";
    public static String VISUALDELAY = "visualdelay";
    public static String VISUALJITTER = "visualjitter";
    public static String VISUALLATENCY = "visuallatency";

    protected boolean updateDeviceHistoryQoSList(String devicename,String valuetype,Object value,int dblistsize){
        if(containsKey(devicename)){
            if(get(devicename).containsKey(valuetype)){
                get(devicename).get(valuetype).add(value);
                if(get(devicename).get(valuetype).size()>dblistsize)
                    for(;get(devicename).get(valuetype).size()>dblistsize;)
                        get(devicename).get(valuetype).remove(0);
            }else{
                get(devicename).put(valuetype,new ArrayList<Object>(){{
                    add(value);
                }});
            }
        }else{
            put(devicename,new HashMap<String,ArrayList<Object>>(){
                {put(valuetype,new ArrayList<Object>(){
                    {add(value);}
                });
                }
            });
