...
import static net.floodlightcontroller.qosoverlldp.internal.QoSoverLLDPManager.PERIODICMODE;
import static net.floodlightcontroller.qosoverlldp.internal.QoSoverLLDPManager.PROACTIVEMODE;
import static net.floodlightcontroller.qosoverlldp.internal.QoSoverLLDPManager.STOCHASTICMODE;

public class DeviceCurrentQoSUpdateModeSelectResource extends ServerResource {
    ...
    public ArrayList<HashMap<String,String>> retrieve() {
        ...
        String mode = (String)getRequestAttributes().get("qos-update-mode");//qos信息更新的模式
        switch(mode){
            case PERIODICMODE:
                qosoverlldp.setDeviceQoSUpdateMode(PERIODICMODE);
                break;
            case PROACTIVEMODE:
                LinkDiscoveryManager.lldpClock = 0;//把lldpclock设置为0,floodlight立刻向所有交换机发送lldp的packet-out请求
                long currenttime = System.currentTimeMillis();
                while(true){
                    if(qosoverlldp.getDeviceCurrentQoSDB().isAllDeviceQoSareAllUpdated(currenttime)) break;
                }
                break;
            case STOCHASTICMODE:
                qosoverlldp.setDeviceQoSUpdateMode(STOCHASTICMODE);
                break;
            default:
                return new ArrayList<HashMap<String,String>>(){{
                    new HashMap<String,String>(){{
                        put("qos-update-mode","please input: periodic or proactive or stochastic");
                    }};
                }};
        }
        Collection<DeviceCurrentQoS> qosvalues = qosoverlldp.getDeviceCurrentQoSDB().values();
        ArrayList<HashMap<String,String>> listmapqos = new ArrayList<>();
        for( DeviceCurrentQoS q : qosvalues){
            listmapqos.add(q.toMap());
        }
        return listmapqos;
    }
