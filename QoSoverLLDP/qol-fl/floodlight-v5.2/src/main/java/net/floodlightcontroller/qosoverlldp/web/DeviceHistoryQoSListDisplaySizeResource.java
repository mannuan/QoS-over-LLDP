...
public class DeviceHistoryQoSListDisplaySizeResource extends ServerResource {
    @Get("json")
    public String retrieve() {
        ...
        int listsize = qosoverlldp.getDeviceHistoryQoSListDBSize();
        int displaylistsize = Integer.parseInt((String) getRequestAttributes().get("device-history-qos-list-display-size"));
        if(qosoverlldp.setDeviceHistoryQoSListDispalySize(displaylistsize)){
            return "Successful devicehistoryqosdbdisplaysize is set to "+displaylistsize;
        }else{
            return "Error:1 < devicehistoryqoslistdisplaysize < "+listsize;
        }
    }
