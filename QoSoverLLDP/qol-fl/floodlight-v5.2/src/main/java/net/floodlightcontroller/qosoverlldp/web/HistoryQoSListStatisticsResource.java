...
public class HistoryQoSListStatisticsResource extends ServerResource {
    ...
    public Object retrieve(){
        ...
        String device = (String)getRequestAttributes().get("device");
        HashMap<String,ArrayList<HashMap<String,String>>> result = new HashMap<>();
        if(devicehistoryqoslistdb.containsKey(device)){
            HashMap<String,ArrayList<Object>> deviceqoslistmap = devicehistoryqoslistdb.get(device);
            ArrayList<HashMap<String,String>> bandwidthlist = new ArrayList<>();
            for(int i=0;i<deviceqoslistmap.get("bandwidth").size();i++){
                final int count = i;
                bandwidthlist.add(new HashMap<String,String>(){{
                    put("period",deviceqoslistmap.get("visualtime").get(count).toString());
                    put("bandwidth",deviceqoslistmap.get("bandwidth").get(count).toString());
                }});
            }
            ArrayList<HashMap<String,String>> delaylist = new ArrayList<>();
            for(int i=0;i<deviceqoslistmap.get("delay").size();i++){
                final int count = i;
                delaylist.add(new HashMap<String,String>(){{
                    put("period",deviceqoslistmap.get("visualtime").get(count).toString());
                    put("delay",deviceqoslistmap.get("delay").get(count).toString());
                }});
            }
            ArrayList<HashMap<String,String>> jitterlist = new ArrayList<>();
            for(int i=0;i<deviceqoslistmap.get("jitter").size();i++){
                final int count = i;
                jitterlist.add(new HashMap<String,String>(){{
                    put("period",deviceqoslistmap.get("visualtime").get(count).toString());
                    put("jitter",deviceqoslistmap.get("jitter").get(count).toString());
                }});
            }
            ArrayList<HashMap<String,String>> losslist = new ArrayList<>();
            for(int i=0;i<deviceqoslistmap.get("loss").size();i++){
                final int count = i;
                losslist.add(new HashMap<String,String>(){{
                    put("period",deviceqoslistmap.get("visualtime").get(count).toString());
                    put("loss",deviceqoslistmap.get("loss").get(count).toString());
                }});
            }
            result.put("bandwidth",bandwidthlist);
            result.put("delay",delaylist);
            result.put("jitter",jitterlist);
