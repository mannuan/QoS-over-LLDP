...
public class ShortestRouteResource extends ServerResource {
	private final String ShortestROUTEFILE = System.getProperty("user.dir")+"/GetShortestRoute.py";
	private final String GetShortestRoutepy =
		"# -*- coding: utf-8 -*-\n" +
		"\"\"\"\n" +
	...
	public String retrieve(){
		try{
			File file =new File(ShortestROUTEFILE);
			if(!file.exists()){
				file.createNewFile();
			}
			...
			bufferWritter.write(GetShortestRoutepy);
			bufferWritter.close();
		}catch(Exception e){
			return e.getMessage();
		}
        ...
        try {  
            Process pro = Runtime.getRuntime().exec(new String[]{"python",
            		ShortestROUTEFILE,src_host,dst_host});
            pro.waitFor();  
