...
public class SwitchPortQoSListforAndroidResource extends ServerResource {
	...
	public List<Map<String,String>> retrieve(){
        ...
        List<Map<String,String>> switchport_qos_list = new ArrayList<>();
        for( DeviceCurrentQoS q : qosvalues){
        	int chassisid = Integer.valueOf(q.getSwitchid().replace(":", ""), 16);
        	if(chassisid == switchid){
        		Map<String,String> switchport_qosmap = new HashMap<>();
        		String switchport = "s"+chassisid+"-eth"+q.getPortid();
        		String qos = "bandwidth:"+q.getvisualBandwidth()+" delay:"+q.getvisualDelay()+" jitter:"+q.getvisualJitter()+
						" loss:"+q.getLoss()+" latency:"+q.getvisualLatency();
        		switchport_qosmap.put("title", switchport);
        		switchport_qosmap.put("detail", qos);
        		switchport_qos_list.add(switchport_qosmap);
        	}
		}
        return switchport_qos_list;
	}
