...
import net.floodlightcontroller.qosoverlldp.internal.DeviceCurrentQoS;
...
public class SwitchQoSVarietyforAndroidResource extends ServerResource {
     ...
     * 计算百分比
     * */
    public String CalculatePercentage(ArrayList<Object> list,String replacement){
        String percentages = new String();//百分比用/隔开
        int len = list.size();
        double sum = 0;
        double arr[] = new double[list.size()];
        for (int i = 0; i < len; i++) {
            arr[i] = Double.parseDouble(list.get(i).toString().replaceAll(replacement, ""))+0.0001;//这里+0.0001的原因是防止在计算百分比是全都为零的情况,而且0.0001很小，对数值本身几乎没有什么影响
            sum += arr[i];
        }
        for (int i = 0; i < len; i++)
            percentages += new BigDecimal(arr[i]/sum*100).setScale(3,BigDecimal.ROUND_HALF_UP).doubleValue()+"/";//对获得的数据保留小数点后3位，四舍五入
        return percentages;
    }

    public ArrayList<Object> AdjustList(ArrayList<Object> list,int requiresize){
        while(list.size()>requiresize){
            list.remove(0);
        }
        return list;
    }
    ...
    public Object retrieve(){
        ...
        int displaysize = qosoverlldp.getDeviceHistoryQoSListDispalySize();
        DeviceHistoryQoSListDB devicehistoryqoslistdb = qosoverlldp.getDeviceHistoryQoSListDB();
        int switchid = Integer.valueOf((String) getRequestAttributes().get("switchid"),10) + 1;
        ArrayList<HashMap> resultlist = new ArrayList<>();
        for(String device:devicehistoryqoslistdb.keySet()){
            if(device.contains("s"+switchid)){
                HashMap<String,ArrayList<Object>> historyqoslistdb = devicehistoryqoslistdb.get(device);
                HashMap<String,String> result = new HashMap<>();
                result.put("title",device);
                String detail = "11/";
                detail+=CalculatePercentage(AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.BANDWIDTH),displaysize), DeviceCurrentQoS.BW_bUNIT);
                detail+=CalculatePercentage(AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.DELAY),displaysize),DeviceCurrentQoS.uUNIT);
                detail+=CalculatePercentage(AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.JITTER),displaysize),DeviceCurrentQoS.uUNIT);
                detail+=CalculatePercentage(AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.LOSS),displaysize),"%");
                detail+=CalculatePercentage(AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.LATENCY),displaysize),DeviceCurrentQoS.uUNIT);
                for(Object value:AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.VISUALTIME),displaysize)){
                    detail+=value+"/";
                }
                for(Object value:AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.VISUALBANDWIDTH),displaysize)){
                    detail+=value+"/";
                }
                for(Object value:AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.VISUALDELAY),displaysize)){
                    detail+=value+"/";
                }
                for(Object value:AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.VISUALJITTER),displaysize)){
                    detail+=value+"/";
                }
                for(Object value:AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.LOSS),displaysize)){
                    detail+=value+"/";
                }
                for(Object value:AdjustList(historyqoslistdb.get(DeviceHistoryQoSListDB.VISUALLATENCY),displaysize)){
                    detail+=value+"/";
                }
                result.put("detail",detail.substring(0,detail.length()-1));
